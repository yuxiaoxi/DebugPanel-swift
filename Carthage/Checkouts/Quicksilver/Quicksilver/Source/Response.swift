...
public final class Response: CustomDebugStringConvertible, Equatable {
  ...
  /// The original URLRequest for this response.
...
public extension Response {
  ...
  /**
   ...
   - throws: `QuicksilverError.statusCode` when others are encountered.
   */
  func filter(statusCodes: ClosedRange<Int>) throws -> Response {
    guard statusCodes.contains(statusCode) else {
      throw QuicksilverError.statusCode(self)
    }
    return self
  }
    ...
    if let keyPath = keyPath {
      ...
      guard let jsonDictionary = try mapJSON() as? NSDictionary,
        let string = jsonDictionary.value(forKeyPath: keyPath) as? String else {
          throw QuicksilverError.stringMapping(self)
      }
      return string
    } else {
  ...
  func mapJSON(failsOnEmptyData: Bool = true) throws -> Any {
    do {
      ...
      throw QuicksilverError.jsonMapping(self)
    }
  }
    ...
    keyPathCheck: if let keyPath = keyPath {
      ...
      if let data = try serializeToData(jsonObject) {
        ...
        do {
          ...
          throw QuicksilverError.objectMapping(error, self)
        }
      }
    } else {
      jsonData = data
