...
char *encodeHexData(char *buff, const char *data, int data_size, BOOL up) {
...
+ (NSString *)encodeHexString:(NSString *)str {
    return [Hex encodeHexData:[str dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (NSString *)decodeHexToString:(NSString *)hex {
    NSData *data = [Hex decodeHexString:hex];
    if (data == nil) {
        return nil;
    }
