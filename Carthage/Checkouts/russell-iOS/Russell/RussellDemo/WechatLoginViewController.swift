...
final class WechatLoginViewController: UIViewController {
  ...
  private func login() {
    ...
    ThirdParty.simpleAuth(forType: .wechat) { response, _ in
      ...
      self.currentSession = Russell.shared?.startOAuthLoginSession(auth: WechatAuth(appID: LLSShareService.wechatId, code: code), delegate: self, isSignup: true, hasUserConfirmedPrivacyInfo: true)
    }
  }
}

extension WechatLoginViewController: OAuthLoginSessionDelegate {
  ...
  func loginSession(_ session: TwoStepRegistrationLoginSession, requiresUserToConfirmRegistrationWithExtraInfo extraInfo: [String : String]?) {
    ...
    let alertController = UIAlertController(title: "确认注册", message: "该微信 \(nick)尚未注册账号，确认注册？", preferredStyle: .alert)
